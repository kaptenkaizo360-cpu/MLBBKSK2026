...
export function defaultStore() {
  ...
  return {
    ...
    // jadual & kedudukan dipaparkan di awam hanya selepas admin sahkan
    published: {}, // cth: { "Sekolah Rendah": true }
  };
}
  ...
  for (const grp of ["A", "B", "C"]) {
    ...
    for (const [a, b] of roundRobin(grpTeams)) {
      matches.push({
        ...
        category, group: grp, order: order++,
...
export function setTeamGroup(store, teamId, newGroup) {
  ...
  // Buang jadual lama kategori ini, jana semula — HANYA pasukan yang tidak dikecualikan
...
// ===== Penerbitan Jadual & Kedudukan (kawalan admin) =====
// Admin mesti SAHKAN secara manual sebelum jadual perlawanan & kedudukan
// dipaparkan di paparan utama / halaman awam.
export function isPublished(store, category) {
  return !!(store.published && store.published[category]);
}

export function setPublished(store, category, value) {
  const published = { ...(store.published || {}), [category]: value };
  return { ...store, published };
}

// Pulihkan pasukan yang telah dibuang dari liga (selamatkan jika tersalah padam)
export function restoreTeam(store, teamId) {
  const team = store.teams.find((t) => t.teamId === teamId);
  if (!team) return store;
  const teams = store.teams.map((t) => t.teamId === teamId ? { ...t, excluded: false } : t);
  const otherMatches = store.matches.filter((m) => m.category !== team.category);
  const regenerated = buildCategoryMatches(teams.filter((t) => !t.excluded && t.category === team.category), team.category, 1);
  const knockout = { ...(store.knockout || {}) };
  delete knockout[team.category];
  return { ...store, teams, matches: [...otherMatches, ...regenerated], knockout };
}

// Bersihkan store: buang TERUS mana-mana pasukan yang bukan dari senarai
// daerah rasmi semasa (cth: daerah yang ditarik balik macam Segamat), dan
// buang sebarang perlawanan yang merujuk pasukan tersebut. Ini dijalankan
// automatik setiap kali data dimuat/disimpan supaya data lama yang tertinggal
// tidak boleh "hidup semula", tanpa perlu admin buat apa-apa secara manual.
export function sanitizeStore(store) {
  if (!store || !Array.isArray(store.teams)) return store;
  const validDistricts = new Set(DISTRICTS.map((d) => d.name));
  const teams = store.teams.filter((t) => validDistricts.has(t.district));
  const validIds = new Set(teams.map((t) => t.teamId));
  let matches = Array.isArray(store.matches)
    ? store.matches.filter((m) => validIds.has(m.teamAId) && validIds.has(m.teamBId))
    : [];

  // Data lama tiada field "order" — isi automatik ikut kedudukan asal dalam kumpulan
  const needsOrder = matches.some((m) => m.order == null);
  if (needsOrder) {
    const counters = {};
    matches = matches.map((m) => {
      ...
      return { ...m, order: counters[key] };
    });
  }

  return { ...store, teams, matches };
}

// Susun semula urutan perlawanan dalam kumpulan yang sama (admin: naik/turun)
export function reorderMatch(store, matchId, direction) {
  const match = store.matches.find((m) => m.matchId === matchId);
  if (!match) return store;

  const groupMatches = store.matches
    .filter((m) => m.category === match.category && m.group === match.group)
    .sort((a, b) => (a.order || 0) - (b.order || 0));

  const idx = groupMatches.findIndex((m) => m.matchId === matchId);
  const swapIdx = direction === "up" ? idx - 1 : idx + 1;
  if (swapIdx < 0 || swapIdx >= groupMatches.length) return store; // sudah di hujung

  const a = groupMatches[idx];
  const b = groupMatches[swapIdx];
  const aOrder = a.order ?? idx + 1;
  const bOrder = b.order ?? swapIdx + 1;

  const matches = store.matches.map((m) => {
    if (m.matchId === a.matchId) return { ...m, order: bOrder };
    if (m.matchId === b.matchId) return { ...m, order: aOrder };
    return m;
  });
  return { ...store, matches };
}

// Tetapkan sendiri pasukan A atau B untuk satu slot perlawanan (admin pilih lawan)
export function setMatchTeams(store, matchId, side, teamId) {
  const team = store.teams.find((t) => t.teamId === teamId);
  if (!team) return store;
  const matches = store.matches.map((m) => {
    if (m.matchId !== matchId) return m;
    if (side === "A") return { ...m, teamA: team.teamName, teamAId: team.teamId };
    return { ...m, teamB: team.teamName, teamBId: team.teamId };
  });
  return { ...store, matches };
}

// Tambah satu slot perlawanan baru dalam kumpulan (admin tetapkan lawan kemudian)
export function addCustomMatch(store, category, group) {
  const groupTeams = store.teams.filter((t) => t.category === category && t.group === group && !t.excluded);
  const a = groupTeams[0];
  const b = groupTeams[1] || groupTeams[0];
  const existing = store.matches.filter((m) => m.category === category && m.group === group);
  const maxOrder = existing.reduce((mx, m) => Math.max(mx, m.order || 0), 0);
  const newMatch = {
    matchId: `M${category === "Sekolah Rendah" ? "SR" : "SM"}-C${Date.now()}`,
    category, group, order: maxOrder + 1,
    teamA: a ? a.teamName : "", teamB: b ? b.teamName : "",
    teamAId: a ? a.teamId : "", teamBId: b ? b.teamId : "",
    winner: "", loser: "", note: "", status: "pending", updatedBy: "",
  };
  return { ...store, matches: [...store.matches, newMatch] };
}

// Padam satu slot perlawanan & nombor semula urutan kumpulan itu
export function deleteMatch(store, matchId) {
  const match = store.matches.find((m) => m.matchId === matchId);
  if (!match) return store;
  let matches = store.matches.filter((m) => m.matchId !== matchId);
  const groupMatches = matches
    .filter((m) => m.category === match.category && m.group === match.group)
    .sort((a, b) => (a.order || 0) - (b.order || 0));
  const renumbered = new Map(groupMatches.map((m, i) => [m.matchId, i + 1]));
  matches = matches.map((m) => renumbered.has(m.matchId) ? { ...m, order: renumbered.get(m.matchId) } : m);
  return { ...store, matches };
}
